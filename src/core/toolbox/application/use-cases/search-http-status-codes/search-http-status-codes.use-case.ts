...
function categorize(code: number): HttpStatusCategory {
...
const HTTP_STATUS_CODES: HttpStatusEntry[] = PHRASES.map(([code, phrase]) => ({
  code,
  phrase,
  category: categorize(code),
}));

export class SearchHttpStatusCodesUseCase {
  execute(query: string): HttpStatusEntry[] {
