...
export class GenerateSvgPlaceholderUseCase {
  execute({
    ...
    const safeBackgroundColor = escapeXml(backgroundColor);
    const safeTextColor = escapeXml(textColor);
    const fontSize = Math.max(12, Math.round(Math.min(width, height) / 8));

    return [
      `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">`,
      `<rect width="${width}" height="${height}" fill="${safeBackgroundColor}" />`,
      `<text x="50%" y="50%" fill="${safeTextColor}" font-family="sans-serif" font-size="${fontSize}" text-anchor="middle" dominant-baseline="middle">${label}</text>`,
      `</svg>`,
    ].join('');
  }
}
