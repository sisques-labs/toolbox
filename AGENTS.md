...
Mirrors [`sisques-labs/gardenia-web`](https://github.com/sisques-labs/gardenia-web), adapted to a backend-less static site: no GraphQL/Zustand/TanStack Query layer exists here (yet), and locale is a client-side toggle (React Context + `localStorage`), not a URL segment.
...
- Locale is a client-side toggle persisted to `localStorage` (`toolbox:locale`), not a URL segment — there's no `[lang]` route segment like gardenia-web.
